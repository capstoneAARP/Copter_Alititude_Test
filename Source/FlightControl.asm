...
SUB	SP, SP, #8
STR	LR, [SP, #0]
;FlightControl.c,102 :: 		current_DC_3 = 6.55;  //Start motors at this value to predict timing-iterations in launch sequence
MOVW	R1, #39322
MOVT	R1, #16593
...
MOVS	R1, #0
BL	_PWM_TIM2_Set_Duty+0
;FlightControl.c,154 :: 		delay_ms(2500);
MOVW	R7, #30783
MOVT	R7, #381
NOP
NOP
...
NOP
NOP
;FlightControl.c,155 :: 		sonarReadValue = alitudeSonarRead();
BL	_alitudeSonarRead+0
; sonarReadValue start address is: 4 (R1)
UXTH	R1, R0
;FlightControl.c,156 :: 		UARTSendString("Sonar average.");
MOVW	R0, #lo_addr(?lstr2_FlightControl+0)
MOVT	R0, #hi_addr(?lstr2_FlightControl+0)
STRH	R1, [SP, #4]
BL	_UARTSendString+0
LDRH	R1, [SP, #4]
;FlightControl.c,157 :: 		UARTSendUint16(sonarReadValue);
UXTH	R0, R1
; sonarReadValue end address is: 4 (R1)
BL	_UARTSendUint16+0
;FlightControl.c,158 :: 		UARTSendNewLine();
BL	_UARTSendNewLine+0
;FlightControl.c,159 :: 		UARTSendString("Throttle");
MOVW	R0, #lo_addr(?lstr3_FlightControl+0)
MOVT	R0, #hi_addr(?lstr3_FlightControl+0)
BL	_UARTSendString+0
;FlightControl.c,160 :: 		UARTSendDouble(current_DC_3);
MOVW	R0, #lo_addr(_current_DC_3+0)
MOVT	R0, #hi_addr(_current_DC_3+0)
LDR	R0, [R0, #0]
BL	_UARTSendDouble+0
;FlightControl.c,161 :: 		return true;
MOVS	R0, #0
;FlightControl.c,162 :: 		}
L_end_TakeOff:
LDR	LR, [SP, #0]
...
; end of _TakeOff
_LoiterMode:
;FlightControl.c,164 :: 		void LoiterMode(){
SUB	SP, SP, #4
STR	LR, [SP, #0]
;FlightControl.c,165 :: 		DC_time = (current_DC_8*pwm_period2)/100;
MOVW	R0, #lo_addr(_pwm_period2+0)
MOVT	R0, #hi_addr(_pwm_period2+0)
LDR	R2, [R0, #0]
...
; end of _LoiterMode
_StabilizeMode:
;FlightControl.c,169 :: 		void StabilizeMode(){
SUB	SP, SP, #4
STR	LR, [SP, #0]
;FlightControl.c,170 :: 		DC_time = (current_DC_3*pwm_period2)/100;
MOVW	R0, #lo_addr(_pwm_period2+0)
MOVT	R0, #hi_addr(_pwm_period2+0)
LDR	R2, [R0, #0]
MOVW	R0, #lo_addr(_current_DC_3+0)
...
_LandingMode:
;FlightControl.c,175 :: 		void LandingMode(){
SUB	SP, SP, #4
STR	LR, [SP, #0]
;FlightControl.c,176 :: 		DC_time = (current_DC_9*pwm_period2)/100;
MOVW	R0, #lo_addr(_pwm_period2+0)
MOVT	R0, #hi_addr(_pwm_period2+0)
LDR	R2, [R0, #0]
...
; end of _LandingMode
_Land:
;FlightControl.c,180 :: 		void Land(){
SUB	SP, SP, #4
STR	LR, [SP, #0]
;FlightControl.c,181 :: 		DC_time = (current_DC_7*pwm_period2)/100;
MOVW	R0, #lo_addr(_pwm_period2+0)
MOVT	R0, #hi_addr(_pwm_period2+0)
LDR	R2, [R0, #0]
...
MOVT	R1, #hi_addr(_DC_time+0)
STRH	R0, [R1, #0]
;FlightControl.c,182 :: 		PWM_TIM3_Set_Duty(DC_time, _PWM_NON_INVERTED, _PWM_CHANNEL1);
MOVS	R2, #0
MOVS	R1, #0
BL	_PWM_TIM3_Set_Duty+0
;FlightControl.c,183 :: 		}
...
; end of _Land
_Yaw_Left:
;FlightControl.c,185 :: 		void Yaw_Left(uint16 timeToYaw_ms){
...
;FlightControl.c,186 :: 		DC_time = (8.0*pwm_period2)/100;
MOVW	R1, #lo_addr(_pwm_period2+0)
MOVT	R1, #hi_addr(_pwm_period2+0)
LDR	R2, [R1, #0]
MOV	R0, #1090519040
...
MOVS	R1, #0
BL	_PWM_TIM2_Set_Duty+0
;FlightControl.c,188 :: 		Vdelay_ms(timeToYaw_ms);
UXTH	R0, R8
; timeToYaw_ms end address is: 32 (R8)
BL	_VDelay_ms+0
;FlightControl.c,189 :: 		Yaw_Stop();
BL	_Yaw_Stop+0
;FlightControl.c,190 :: 		}
L_end_Yaw_Left:
LDR	LR, [SP, #0]
...
; end of _Yaw_Left
_Yaw_Right:
;FlightControl.c,192 :: 		void Yaw_Right(const uint16 timeToYaw_ms){
; timeToYaw_ms start address is: 0 (R0)
SUB	SP, SP, #4
STR	LR, [SP, #0]
UXTH	R8, R0
; timeToYaw_ms end address is: 0 (R0)
; timeToYaw_ms start address is: 32 (R8)
;FlightControl.c,193 :: 		DC_time = (6.8*pwm_period2)/100;
MOVW	R1, #lo_addr(_pwm_period2+0)
MOVT	R1, #hi_addr(_pwm_period2+0)
LDR	R2, [R1, #0]
MOVW	R0, #39322
...
MOVT	R1, #hi_addr(_DC_time+0)
STRH	R0, [R1, #0]
;FlightControl.c,194 :: 		PWM_TIM2_Set_Duty(DC_time, _PWM_NON_INVERTED, _PWM_CHANNEL2);       //for YAW
UXTH	R1, R0
MOVS	R2, #1
UXTH	R0, R1
MOVS	R1, #0
BL	_PWM_TIM2_Set_Duty+0
;FlightControl.c,195 :: 		Vdelay_ms(timeToYaw_ms);
UXTH	R0, R8
; timeToYaw_ms end address is: 32 (R8)
BL	_VDelay_ms+0
;FlightControl.c,196 :: 		Yaw_Stop();
BL	_Yaw_Stop+0
;FlightControl.c,197 :: 		}
L_end_Yaw_Right:
LDR	LR, [SP, #0]
...
; end of _Yaw_Right
_Yaw_Stop:
;FlightControl.c,199 :: 		void Yaw_Stop(){
SUB	SP, SP, #4
STR	LR, [SP, #0]
;FlightControl.c,200 :: 		DC_time = (current_DC*pwm_period2)/100;
MOVW	R0, #lo_addr(_pwm_period2+0)
MOVT	R0, #hi_addr(_pwm_period2+0)
LDR	R2, [R0, #0]
MOVW	R0, #lo_addr(_current_DC+0)
...
MOVT	R1, #hi_addr(_DC_time+0)
STRH	R0, [R1, #0]
;FlightControl.c,201 :: 		PWM_TIM2_Set_Duty(DC_time, _PWM_NON_INVERTED, _PWM_CHANNEL2);       //for YAW
MOVS	R2, #1
MOVS	R1, #0
BL	_PWM_TIM2_Set_Duty+0
;FlightControl.c,202 :: 		}
...
; end of _Yaw_Stop
_Forward_Flight:
;FlightControl.c,204 :: 		void Forward_Flight(){
SUB	SP, SP, #4
STR	LR, [SP, #0]
;FlightControl.c,205 :: 		DC_time = (6.5*pwm_period2)/100;
MOVW	R0, #lo_addr(_pwm_period2+0)
MOVT	R0, #hi_addr(_pwm_period2+0)
LDR	R2, [R0, #0]
MOVW	R0, #0
...
_Stop_Forward:
;FlightControl.c,209 :: 		void Stop_Forward(){
SUB	SP, SP, #4
STR	LR, [SP, #0]
;FlightControl.c,210 :: 		DC_time = (current_DC*pwm_period2)/100;
MOVW	R0, #lo_addr(_pwm_period2+0)
MOVT	R0, #hi_addr(_pwm_period2+0)
LDR	R2, [R0, #0]
MOVW	R0, #lo_addr(_current_DC+0)
...
MOVT	R1, #hi_addr(_DC_time+0)
STRH	R0, [R1, #0]
;FlightControl.c,211 :: 		PWM_TIM2_Set_Duty(DC_time, _PWM_NON_INVERTED, _PWM_CHANNEL3);       //for PITCH
MOVS	R2, #2
MOVS	R1, #0
BL	_PWM_TIM2_Set_Duty+0
;FlightControl.c,212 :: 		}
...
_sonarGeneric:
;FlightControl.c,214 :: 		uint16 sonarGeneric()
SUB	SP, SP, #4
STR	LR, [SP, #0]
;FlightControl.c,216 :: 		uint16 sonar = ADC1_Get_Sample(13);
MOVS	R0, #13
BL	_ADC1_Get_Sample+0
;FlightControl.c,217 :: 		sonar >>= 4;
LSRS	R0, R0, #4
;FlightControl.c,218 :: 		return sonar;
;FlightControl.c,219 :: 		}
L_end_sonarGeneric:
LDR	LR, [SP, #0]
ADD	SP, SP, #4
...
_alitudeSonarRead:
;FlightControl.c,221 :: 		uint16 alitudeSonarRead()
SUB	SP, SP, #20
STR	LR, [SP, #0]
;FlightControl.c,223 :: 		uint16 sonarArray[SONAR_ITERATIONS] = {0};
ADD	R11, SP, #4
ADD	R10, R11, #8
MOVW	R12, #lo_addr(?ICSalitudeSonarRead_sonarArray_L0+0)
MOVT	R12, #hi_addr(?ICSalitudeSonarRead_sonarArray_L0+0)
BL	___CC2DW+0
;FlightControl.c,224 :: 		int16 i = 0;
;FlightControl.c,225 :: 		uint32 sonarAvg = 0;
; sonarAvg start address is: 32 (R8)
MOV	R8, #0
;FlightControl.c,226 :: 		uint32 secondAvg = 0;
;FlightControl.c,227 :: 		uint8 anomolyCount = 0;
; anomolyCount start address is: 24 (R6)
MOVS	R6, #0
;FlightControl.c,230 :: 		for(i=0; i < SONAR_ITERATIONS; i++)
...
;FlightControl.c,232 :: 		sonarArray[i] = ADC1_Get_Sample(13);                          // Get ADC value from corresponding channel
ADD	R1, SP, #4
STR	R1, [SP, #16]
LSLS	R0, R5, #1
ADDS	R0, R1, R0
...
LDR	R1, [SP, #12]
STRH	R0, [R1, #0]
;FlightControl.c,233 :: 		sonarArray[i] >>= 4;
LSLS	R0, R5, #1
LDR	R2, [SP, #16]
ADDS	R1, R2, R0
LDRH	R0, [R1, #0]
LSRS	R0, R0, #4
STRH	R0, [R1, #0]
;FlightControl.c,235 :: 		if(sonarArray[i] >= SONAR_MAX_VALUE)
LSLS	R0, R5, #1
ADDS	R0, R2, R0
LDRH	R0, [R0, #0]
CMP	R0, #240
...
;FlightControl.c,238 :: 		i--;
SUBS	R1, R5, #1
SXTH	R1, R1
; i end address is: 20 (R5)
; i start address is: 4 (R1)
;FlightControl.c,239 :: 		anomolyCount++;
ADDS	R0, R6, #1
UXTB	R0, R0
UXTB	R6, R0
;FlightControl.c,240 :: 		if(anomolyCount > MAX_ANOMOLY_TOSS)
CMP	R0, #10
IT	LS
...
;FlightControl.c,242 :: 		return 255;
MOVS	R0, #255
IT	AL
...
;FlightControl.c,247 :: 		sonarAvg += sonarArray[i];
; i start address is: 20 (R5)
ADD	R1, SP, #4
LSLS	R0, R5, #1
ADDS	R0, R1, R0
...
;FlightControl.c,249 :: 		Delay_ms(ALITUDE_SONAR_READ_DELAY);
; sonarAvg start address is: 32 (R8)
; anomolyCount start address is: 24 (R6)
; i start address is: 4 (R1)
MOVW	R7, #51423
...
;FlightControl.c,251 :: 		sonarAvg = sonarAvg/SONAR_ITERATIONS;
LSR	R2, R8, #2
; sonarAvg end address is: 32 (R8)
; sonarAvg start address is: 8 (R2)
;FlightControl.c,252 :: 		secondAvg = 0;
; secondAvg start address is: 16 (R4)
MOVS	R4, #0
;FlightControl.c,253 :: 		for(i=0; i < SONAR_ITERATIONS; i++)
...
; sonarAvg end address is: 8 (R2)
;FlightControl.c,255 :: 		if(sonarArray[i] >= sonarAvg + SONAR_OUTLIER_OFFSET)
; sonarAvg start address is: 8 (R2)
ADD	R1, SP, #4
LSLS	R0, R3, #1
ADDS	R0, R1, R0
...
;FlightControl.c,257 :: 		sonarArray[i] = sonarAvg;
ADD	R1, SP, #4
LSLS	R0, R3, #1
ADDS	R0, R1, R0
STRH	R2, [R0, #0]
...
;FlightControl.c,259 :: 		secondAvg += sonarArray[i];
ADD	R1, SP, #4
LSLS	R0, R3, #1
ADDS	R0, R1, R0
LDRH	R0, [R0, #0]
...
;FlightControl.c,261 :: 		return((uint16)secondAvg/SONAR_ITERATIONS);
UXTH	R0, R4
; secondAvg end address is: 16 (R4)
LSRS	R0, R0, #2
;FlightControl.c,262 :: 		}
...
_Stabilize_Alt:
;FlightControl.c,264 :: 		void Stabilize_Alt()
SUB	SP, SP, #16
STR	LR, [SP, #0]
;FlightControl.c,266 :: 		uint16 sonarAlititude = 0;
;FlightControl.c,267 :: 		uint8 failSafeCounter = 0;
MOVS	R0, #0
STRB	R0, [SP, #6]
MOVS	R0, #0
STRB	R0, [SP, #7]
;FlightControl.c,268 :: 		uint8 sonarReadIteration = 0;
;FlightControl.c,275 :: 		UARTSendString("Stablilizing Alititude.");
MOVW	R0, #lo_addr(?lstr4_FlightControl+0)
MOVT	R0, #hi_addr(?lstr4_FlightControl+0)
BL	_UARTSendString+0
;FlightControl.c,276 :: 		sonarAlititude = alitudeSonarRead();
BL	_alitudeSonarRead+0
STRH	R0, [SP, #4]
;FlightControl.c,277 :: 		UARTSendString("1st Sonar average.");
MOVW	R0, #lo_addr(?lstr5_FlightControl+0)
MOVT	R0, #hi_addr(?lstr5_FlightControl+0)
BL	_UARTSendString+0
;FlightControl.c,278 :: 		UARTSendUint16(sonarAlititude);
LDRH	R0, [SP, #4]
BL	_UARTSendUint16+0
;FlightControl.c,279 :: 		UARTSendNewLine();
BL	_UARTSendNewLine+0
;FlightControl.c,281 :: 		while((sonarAlititude < (ALTITUDE_HOLD - SONAR_ALITUDE_RANGE)) || (sonarAlititude > (ALTITUDE_HOLD + SONAR_ALITUDE_RANGE)))
L_Stabilize_Alt28:
LDRH	R0, [SP, #4]
CMP	R0, #64
IT	CC
...
L__Stabilize_Alt40:
L__Stabilize_Alt39:
;FlightControl.c,283 :: 		if(failSafeCounter >= ALTITUDE_FAIL_SAFE_MAX)
LDRB	R0, [SP, #6]
CMP	R0, #10
IT	CC
BCC	L_Stabilize_Alt32
;FlightControl.c,285 :: 		UARTSendString("Breaking out, too many iterations.");
MOVW	R0, #lo_addr(?lstr6_FlightControl+0)
MOVT	R0, #hi_addr(?lstr6_FlightControl+0)
BL	_UARTSendString+0
;FlightControl.c,286 :: 		return;
IT	AL
BAL	L_end_Stabilize_Alt
;FlightControl.c,287 :: 		}
L_Stabilize_Alt32:
;FlightControl.c,288 :: 		else if(sonarReadIteration >= ALITITUDE_SONAR_READ_ITER)
LDRB	R0, [SP, #7]
CMP	R0, #3
IT	CC
BCC	L_Stabilize_Alt34
;FlightControl.c,290 :: 		sonarReadIteration = 0;
MOVS	R0, #0
STRB	R0, [SP, #7]
;FlightControl.c,291 :: 		if(sonarAlititude > ALTITUDE_HOLD)
LDRH	R0, [SP, #4]
CMP	R0, #72
IT	LS
BLS	L_Stabilize_Alt35
;FlightControl.c,293 :: 		current_DC_3 -= ALT_THROTLE_STEP_SIZE;
MOVW	R0, #lo_addr(_current_DC_3+0)
MOVT	R0, #hi_addr(_current_DC_3+0)
STR	R0, [SP, #12]
LDR	R0, [R0, #0]
...
LDR	R1, [SP, #12]
STR	R0, [R1, #0]
;FlightControl.c,294 :: 		UARTSendString("Decrease Throttle.");
...
L_Stabilize_Alt35:
;FlightControl.c,296 :: 		else if(sonarAlititude < ALTITUDE_HOLD)
LDRH	R0, [SP, #4]
CMP	R0, #72
IT	CS
BCS	L_Stabilize_Alt37
;FlightControl.c,298 :: 		current_DC_3 += ALT_THROTLE_STEP_SIZE;
MOVW	R0, #lo_addr(_current_DC_3+0)
MOVT	R0, #hi_addr(_current_DC_3+0)
STR	R0, [SP, #12]
LDR	R2, [R0, #0]
...
LDR	R1, [SP, #12]
STR	R0, [R1, #0]
;FlightControl.c,299 :: 		UARTSendString("Increase Throttle.");
...
L_Stabilize_Alt37:
L_Stabilize_Alt36:
;FlightControl.c,301 :: 		GPIOC_ODR.B8 = ~GPIOC_ODR.B8;
...
MOVT	R0, #hi_addr(GPIOC_ODR+0)
STR	R1, [R0, #0]
;FlightControl.c,302 :: 		UARTSendString("Throttle:");
MOVW	R0, #lo_addr(?lstr9_FlightControl+0)
MOVT	R0, #hi_addr(?lstr9_FlightControl+0)
BL	_UARTSendString+0
;FlightControl.c,303 :: 		UARTSendDouble(current_DC_3);
MOVW	R0, #lo_addr(_current_DC_3+0)
MOVT	R0, #hi_addr(_current_DC_3+0)
LDR	R0, [R0, #0]
BL	_UARTSendDouble+0
;FlightControl.c,305 :: 		DC_time = (current_DC_3*pwm_period2)/100;
MOVW	R0, #lo_addr(_pwm_period2+0)
MOVT	R0, #hi_addr(_pwm_period2+0)
LDR	R2, [R0, #0]
MOVW	R0, #lo_addr(_current_DC_3+0)
...
MOVT	R1, #hi_addr(_DC_time+0)
STRH	R0, [R1, #0]
;FlightControl.c,306 :: 		PWM_TIM2_Set_Duty(DC_time, _PWM_NON_INVERTED, _PWM_CHANNEL1);
MOVS	R2, #0
MOVS	R1, #0
BL	_PWM_TIM2_Set_Duty+0
;FlightControl.c,307 :: 		failSafeCounter++;
LDRB	R0, [SP, #6]
ADDS	R0, R0, #1
STRB	R0, [SP, #6]
;FlightControl.c,308 :: 		}
L_Stabilize_Alt34:
;FlightControl.c,310 :: 		sonarAlititude = alitudeSonarRead();
BL	_alitudeSonarRead+0
STRH	R0, [SP, #4]
;FlightControl.c,311 :: 		UARTSendUint16(sonarAlititude);
BL	_UARTSendUint16+0
;FlightControl.c,312 :: 		sonarReadIteration++;
LDRB	R0, [SP, #7]
ADDS	R0, R0, #1
STRB	R0, [SP, #7]
;FlightControl.c,313 :: 		}
...
L_Stabilize_Alt29:
;FlightControl.c,314 :: 		UARTSendString("Reached Altitude.");
