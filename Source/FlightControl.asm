...
_alitudeSonarRead:
;FlightControl.c,214 :: 		uint16 alitudeSonarRead()
SUB	SP, SP, #20
STR	LR, [SP, #0]
;FlightControl.c,216 :: 		uint16 sonarArray[SONAR_ITERATIONS] = {0};
ADD	R11, SP, #4
ADD	R10, R11, #8
MOVW	R12, #lo_addr(?ICSalitudeSonarRead_sonarArray_L0+0)
MOVT	R12, #hi_addr(?ICSalitudeSonarRead_sonarArray_L0+0)
BL	___CC2DW+0
;FlightControl.c,217 :: 		int16 i = 0;
;FlightControl.c,218 :: 		uint32 sonarAvg = 0;
; sonarAvg start address is: 32 (R8)
MOV	R8, #0
;FlightControl.c,219 :: 		uint32 secondAvg = 0;
;FlightControl.c,220 :: 		uint8 anomolyCount = 0;
; anomolyCount start address is: 24 (R6)
MOVS	R6, #0
;FlightControl.c,223 :: 		for(i=0; i < SONAR_ITERATIONS; i++)
...
;FlightControl.c,225 :: 		sonarArray[i] = ADC1_Get_Sample(13);                          // Get ADC value from corresponding channel
ADD	R1, SP, #4
STR	R1, [SP, #16]
LSLS	R0, R5, #1
ADDS	R0, R1, R0
...
LDR	R1, [SP, #12]
STRH	R0, [R1, #0]
;FlightControl.c,226 :: 		sonarArray[i] >>= 4;
LSLS	R0, R5, #1
LDR	R2, [SP, #16]
ADDS	R1, R2, R0
LDRH	R0, [R1, #0]
LSRS	R0, R0, #4
STRH	R0, [R1, #0]
;FlightControl.c,228 :: 		if(sonarArray[i] >= SONAR_MAX_VALUE)
LSLS	R0, R5, #1
ADDS	R0, R2, R0
LDRH	R0, [R0, #0]
CMP	R0, #240
...
;FlightControl.c,231 :: 		i--;
SUBS	R1, R5, #1
SXTH	R1, R1
; i end address is: 20 (R5)
; i start address is: 4 (R1)
;FlightControl.c,232 :: 		anomolyCount++;
ADDS	R0, R6, #1
UXTB	R0, R0
UXTB	R6, R0
;FlightControl.c,233 :: 		if(anomolyCount > MAX_ANOMOLY_TOSS)
CMP	R0, #10
IT	LS
...
;FlightControl.c,235 :: 		return 255;
MOVS	R0, #255
IT	AL
...
;FlightControl.c,240 :: 		sonarAvg += sonarArray[i];
; i start address is: 20 (R5)
ADD	R1, SP, #4
LSLS	R0, R5, #1
ADDS	R0, R1, R0
...
;FlightControl.c,242 :: 		Delay_ms(ALITUDE_SONAR_READ_DELAY);
; sonarAvg start address is: 32 (R8)
; anomolyCount start address is: 24 (R6)
; i start address is: 4 (R1)
MOVW	R7, #51423
...
;FlightControl.c,244 :: 		sonarAvg = sonarAvg/SONAR_ITERATIONS;
LSR	R2, R8, #2
; sonarAvg end address is: 32 (R8)
; sonarAvg start address is: 8 (R2)
;FlightControl.c,245 :: 		secondAvg = 0;
; secondAvg start address is: 16 (R4)
MOVS	R4, #0
;FlightControl.c,246 :: 		for(i=0; i < SONAR_ITERATIONS; i++)
...
; sonarAvg end address is: 8 (R2)
;FlightControl.c,248 :: 		if(sonarArray[i] >= sonarAvg + SONAR_OUTLIER_OFFSET)
; sonarAvg start address is: 8 (R2)
ADD	R1, SP, #4
LSLS	R0, R3, #1
ADDS	R0, R1, R0
...
;FlightControl.c,250 :: 		sonarArray[i] = sonarAvg;
ADD	R1, SP, #4
LSLS	R0, R3, #1
ADDS	R0, R1, R0
STRH	R2, [R0, #0]
...
;FlightControl.c,252 :: 		secondAvg += sonarArray[i];
ADD	R1, SP, #4
LSLS	R0, R3, #1
ADDS	R0, R1, R0
LDRH	R0, [R0, #0]
...
;FlightControl.c,254 :: 		return((uint16)secondAvg/SONAR_ITERATIONS);
UXTH	R0, R4
; secondAvg end address is: 16 (R4)
LSRS	R0, R0, #2
;FlightControl.c,255 :: 		}
...
_Stabilize_Alt:
;FlightControl.c,257 :: 		void Stabilize_Alt()
SUB	SP, SP, #16
STR	LR, [SP, #0]
;FlightControl.c,259 :: 		uint16 sonarAlititude = 0;
;FlightControl.c,260 :: 		uint8 failSafeCounter = 0;
MOVS	R0, #0
STRB	R0, [SP, #6]
MOVS	R0, #0
STRB	R0, [SP, #7]
;FlightControl.c,261 :: 		uint8 sonarReadIteration = 0;
...
;FlightControl.c,286 :: 		current_DC_3 -= ALT_THROTLE_STEP_SIZE;
MOVW	R0, #lo_addr(_current_DC_3+0)
MOVT	R0, #hi_addr(_current_DC_3+0)
STR	R0, [SP, #12]
LDR	R0, [R0, #0]
...
;FlightControl.c,291 :: 		current_DC_3 += ALT_THROTLE_STEP_SIZE;
MOVW	R0, #lo_addr(_current_DC_3+0)
MOVT	R0, #hi_addr(_current_DC_3+0)
STR	R0, [SP, #12]
LDR	R2, [R0, #0]
